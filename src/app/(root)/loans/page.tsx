...
const LOAN_CATEGORIES = new Set<Category>([
  "debt_personal_loan",
  "debt_student_loan",
  "debt_medical",
  "debt_bnpl",
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setItems((data.items ?? []).filter((txn) => LOAN_CATEGORIES.has(txn.category as Category)));
        }
      } catch {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Dedicated view of loan-related debt payment activity.</p>
        </div>
      ...
      <div className="rounded-xl bg-arcana-surface border border-arcana-border p-5">
        ...
        <p className="text-2xl font-bold text-cyan-300">{formatCurrency(total)}</p>
      ...
      <div className="rounded-xl bg-arcana-surface border border-arcana-border overflow-hidden">
        ...
        {loading ? (
          <div className="px-5 py-8 text-sm text-slate-400">Loading loan transactions...</div>
        ) : items.length === 0 ? (
          <div className="px-5 py-8 text-sm text-slate-500">No loan transactions found.</div>
        ) : (
