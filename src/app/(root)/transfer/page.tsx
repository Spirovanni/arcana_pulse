...
export default function TransferPage() {
  ...
  // In-memory transfer list (will be replaced by transfer service in arcana_pulse-ban)
  ...
  function validate(): boolean {
    ...
    } else if (!/^ARC-[A-Z0-9]+-\d{4}$/.test(form.receiverShareableId.trim())) {
  ...
  function handleConfirm() {
    setSubmitting(true);
    // Mock submission — will be replaced by transfer service
    const now = new Date().toISOString();
    const newTransfer: Transfer = {
      transferId: `xfr-${Date.now().toString(36)}`,
          ...
          <form className="space-y-4" onSubmit={handleReview}>
            <div>
              ...
              <select
                ...
                onChange={(e) => updateField("senderBankId", e.target.value)}
              ...
              {errors.senderBankId && (
                <p className="text-xs text-arcana-danger mt-1">{errors.senderBankId}</p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateField("receiverShareableId", e.target.value.toUpperCase())}
              ...
              {errors.receiverShareableId ? (
                <p className="text-xs text-arcana-danger mt-1">{errors.receiverShareableId}</p>
              ) : (
                <p className="text-xs text-slate-500 mt-1">
                  The recipient&apos;s unique Arcana Transfer ID shown on their bank card
                </p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.recipientEmail && (
                <p className="text-xs text-arcana-danger mt-1">{errors.recipientEmail}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.amount && (
                <p className="text-xs text-arcana-danger mt-1">{errors.amount}</p>
              )}
            </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm text-white font-medium">
                          {xfrSenderBank?.institutionName || "Unknown"} &rarr;{" "}
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-white font-medium">
                  {senderBank?.institutionName} (...{senderBank?.displayMask})
                </span>
              </div>
              <div className="flex justify-between text-sm">
